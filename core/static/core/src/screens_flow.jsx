...
function RequisicaoDetail({ id, setRoute, role }) {
  ...
  function aprovar() {
    ...
    window.API.aprovarRequisicao(req.id).catch(console.error);
    flash("Requisição aprovada — estoque reservado nas localizações com saldo livre.");
  }
  function rejeitar() {
    setReq((r) => ({ ...r, status: "rejeitada" }));
    window.API.rejeitarRequisicao(req.id).catch(console.error);
    flash("Requisição rejeitada.");
  }
  function atender(itemId) {
    ...
    window.API.atenderRequisicaoItem(req.id, itemId).catch(console.error);
...
function ReservasScreen({ role }) {
  const [reservas, setReservas] = React.useState(RESERVAS);

  async function confirmar(id) {
    try {
      await window.API.confirmarReserva(id);
      setReservas((rs) => rs.map((r) => r.id === id ? { ...r, status: "confirmada" } : r));
      ...
      <DataTable columns={[{ label: "Equipamento" }, { label: "Solicitante" }, { label: "Início" }, { label: "Fim" }, { label: "Finalidade" }, { label: "Status" }, ...(role === "gestor" ? [{ label: "", align: "right" }] : [])]}>
        {reservas.map((r) => (
          <Row key={r.id}>
            ...
            {role === "gestor" && <td className="px-4 py-3 text-right">{r.status === "pendente" ? <Button size="sm" variant="outline" icon="check" onClick={() => confirmar(r.id)}>Confirmar</Button> : null}</td>}
          </Row>
        ))}
      </DataTable>
    </div>
